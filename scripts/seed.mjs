...
const SEED_RESTAURANTS = [
  { name: 'Skyline Burgers', address: '12 Marina Way, Lagos', latitude: '6.4501', longitude: '3.3947' },
  { name: 'Cloud Kitchen Pizza', address: '34 Allen Ave, Ikeja', latitude: '6.6018', longitude: '3.3515' },
  { name: 'Drone Sushi', address: '78 Admiralty Way, Lekki', latitude: '6.4283', longitude: '3.4583' },
];

const MENUS = {
  'Skyline Burgers': [
    { name: 'Cheeseburger', price: '4500.00', description: 'Beef patty + cheddar' },
    { name: 'Chicken Burger', price: '4200.00', description: 'Crispy chicken thigh' },
    { name: 'Sky Fries', price: '1500.00', description: 'Salted hand-cut fries' },
    { name: 'Iced Tea', price: '1200.00', description: 'Lemon iced tea' },
    { name: 'Veggie Wrap', price: '3800.00', description: 'Garden veggies + hummus' },
  ],
  'Cloud Kitchen Pizza': [
    { name: 'Margherita', price: '6500.00', description: 'Tomato, mozzarella, basil' },
    { name: 'Pepperoni', price: '7200.00', description: 'Beef pepperoni + cheese' },
    { name: 'BBQ Chicken', price: '7500.00', description: 'BBQ sauce + grilled chicken' },
    { name: 'Vegetarian', price: '6800.00', description: 'Peppers, mushrooms, onions' },
    { name: 'Coke 50cl', price: '900.00', description: 'Chilled' },
  ],
  'Drone Sushi': [
    { name: 'Salmon Nigiri (6)', price: '5500.00', description: 'Fresh salmon over rice' },
    { name: 'California Roll', price: '4800.00', description: 'Crab + avocado + cucumber' },
    { name: 'Spicy Tuna Roll', price: '5200.00', description: 'With sriracha mayo' },
...
async function upsertRestaurant(client, ownerId, restaurant) {
  const existing = await client.query(`SELECT id FROM restaurants WHERE name = $1 AND owner_id = $2`, [
    restaurant.name,
    ownerId,
  ]);
  if (existing.rows[0]) {
  ...
  const res = await client.query(
    `INSERT INTO restaurants (owner_id, name, address, latitude, longitude)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`,
    [ownerId, restaurant.name, restaurant.address, restaurant.latitude, restaurant.longitude],
  );
  return res.rows[0];
}
...
async function upsertDrone(client, drone) {
  const existing = await client.query(`SELECT id FROM drones WHERE code = $1`, [drone.code]);
  if (existing.rows[0]) {
    return existing.rows[0];
  }
  const res = await client.query(
